...
void MainWindow::ensureMenu(){ if(!menu_) menu_ = std::make_unique<TopMenu>(this); }
void MainWindow::ensureCenter(){ if(!center_) center_ = std::make_unique<CenterStack>(this); }
void MainWindow::ensureNav(){ if(!nav_) nav_ = std::make_unique<NavDock>(this); }
...
void MainWindow::wire() {
    ...
    connect(center_.get(), &CenterStack::compareRequested, this,
            [=](const QString& L, const QString& R){
                if (logm_) logm_->append(QString("[UI] Compare: %1 | %2").arg(L, R));
