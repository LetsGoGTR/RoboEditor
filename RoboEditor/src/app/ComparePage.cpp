...
{
    if (rightSplit_) return rightSplit_->saveState();
    return QByteArray();
}

void ComparePage::restoreSplitterState(const QByteArray &state)
{
    if (rightSplit_ && !state.isEmpty()) rightSplit_->restoreState(state);
}
...
QWidget *ComparePage::buildRightPanel()
{
    ...
    connect(btnSelectFolder, &QPushButton::clicked, this, [this]() {
        ...
        if (listView) listView->setSelectionMode(QAbstractItemView::SingleSelection);
    ...
    if (!diffPanel_) {
        ...
        // 시그널 연결
        connect(diffPanel_, &DiffTablePanel::fileDoubleClicked,
                this, &ComparePage::onFolderFileClicked);
    }
...
void ComparePage::clearHighlights()
{
    ...
    if (leftDiffHighlighter_) leftDiffHighlighter_->clearLineStates();
    if (rightDiffHighlighter_) rightDiffHighlighter_->clearLineStates();
}

QString ComparePage::formatPathForCompare(const QString &fullPath) const
{
    if (fullPath.isEmpty()) return "No file opened";
    QString displayPath = fullPath;
    if (displayPath.startsWith("C:/backup", Qt::CaseInsensitive)) displayPath.remove(0, 9);
    else if (displayPath.startsWith("C:\\backup", Qt::CaseInsensitive)) displayPath.remove(0, 9);
    if (displayPath.startsWith('/') || displayPath.startsWith('\\')) displayPath.remove(0, 1);
    displayPath.replace('/', " > ");
    displayPath.replace('\\', " > ");
    return displayPath;
}
...
{
    if (path.isEmpty() || !compareTabWidget_) return;
    ...
    pathLabel->setStyleSheet("QLabel { padding: 4px 8px; background-color: #f0f0f0; border-bottom: 1px solid #d0d0d0; font-size: 9pt; color: #666; }");
...
{
    if (!compareTabWidget_ || compareTabWidget_->count() == 0) return;
    ...
    if (!effectiveLeftPath.isEmpty() && !FileTypeHelper::isCompatible(lType, rType)) {
        ...
        if (diffPanel_) {
            diffPanel_->setFileTypeInfo(leftTypeName,
                                        rightTypeName,
                                        false,
                                        tr("호환되지 않음"));
        }
    ...
    // 2. Diff 수행
    services::ServiceResult result = services::DiffService::diff(
            rightText.toStdString(), leftText.toStdString(),
            targetPath_.toStdString(), effectiveLeftPath.toStdString());
    ...
    if (!result.success) {
        ...
        if (fileTypeStr == "yaml") {
            ...
            // 좌측 파싱 시도
            try { YAML::Load(leftText.toStdString()); }
            ...
            // 우측 파싱 시도
            if (!errorFound) {
                try { YAML::Load(rightText.toStdString()); }
                catch(const YAML::Exception &e) {
                    errorLine = e.mark.line + 1;
                    errorFound = true;
                    errorDetail = QString::fromStdString(e.msg);
    ...
    if (diffRows.isEmpty() && fileTypeStr == "yaml") {
        ...
        try { YAML::Load(leftText.toStdString()); }
        catch(const YAML::Exception &e) { errorLine = e.mark.line + 1; errorFound = true; }
...
QList<DiffRow> ComparePage::parseDiffResult(const Json::Value &result, const QString &fileType)
{
    ...
    try {
        ...
        if (!changes.isArray()) return rows;
        ...
        for (const auto &change : changes) {
            ...
            if (change.isMember("type")) {
                ...
                if (t == "added") row.state = "ADDED";
                else if (t == "removed") row.state = "REMOVED";
                else if (t == "modified") row.state = "CHANGED";
                else row.state = "SAME";
            }
            ...
            if (fileType == "yaml") {
                ...
                if (change.isMember("oldLineNumber") && !change["oldLineNumber"].isNull()) {
                    ...
                    if (row.line == -1) row.line = row.rightLineNumber;
                }
                if (change.isMember("path")) row.key = QString::fromStdString(change["path"].asString());
                ...
                if (row.state == "ADDED") row.leftLineNumber = row.line;
                else if (row.state == "REMOVED") row.rightLineNumber = row.line;
                else { row.leftLineNumber = row.rightLineNumber = row.line; }
            }
            rows.append(row);
        }
    } catch (...) {}
...
void ComparePage::performDiff(const QString &leftPath, const QString &rightPath)
{
    ...
    if (!leftFile.open(QIODevice::ReadOnly | QIODevice::Text)) return;
...
void ComparePage::performFolderDiff(const QString &leftPath, const QString &rightPath)
{
    ...
    if (lPath.isEmpty() || rPath.isEmpty()) return;
...
QString ComparePage::extractArchiveToTemp(const QString &archivePath)
{
    ...
    if (!result.success) return QString();
...
void ComparePage::cleanupTempFolders()
{
    for (const QString &folder : tempFolders_) {
        if (QDir(folder).exists()) QDir(folder).removeRecursively();
    }
    tempFolders_.clear();
}
...
ComparePage::ControllerPathInfo ComparePage::extractControllerInfo(const QString &path) const
{
    ...
    for (int i=0; i<parts.size(); ++i) {
        if (parts[i].compare("backup", Qt::CaseInsensitive) == 0) {
            if (i+1 < parts.size()) info.serial = parts[i+1];
            break;
        }
    }
    return info;
...
QPair<QString, QString> ComparePage::determineColumnHeaders(const QString &leftPath, const QString &rightPath) const
{
    ControllerPathInfo leftInfo = extractControllerInfo(leftPath);
    ControllerPathInfo rightInfo = extractControllerInfo(rightPath);

    QString leftHeader = leftInfo.serial.isEmpty() ? QFileInfo(leftPath).fileName() : leftInfo.serial;
    QString rightHeader = rightInfo.serial.isEmpty() ? QFileInfo(rightPath).fileName() : rightInfo.serial;
