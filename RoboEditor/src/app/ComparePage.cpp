...
void ComparePage::recalcDiff(const QString &leftText, const QString &leftPath)
{
    ...
    services::ServiceResult result =
            services::DiffService::diff(rightContentStr, leftContentStr, rightNameStr, leftNameStr);
...
void ComparePage::refreshDiffTable(const QList<DiffRow> &rows)
{
    ...
    for (int i = 0; i < rows.size(); i++) {
        ...
        // 텍스트 정렬
        if (itemLine)
            itemLine->setTextAlignment(Qt::AlignCenter | Qt::AlignVCenter);
...
void ComparePage::performDiff(const QString &leftPath, const QString &rightPath)
{
    ...
    if (!rightFile.open(QIODevice::ReadOnly | QIODevice::Text)) {
        QMessageBox::critical(
                this, tr("오류"), tr("오른쪽 파일을 열 수 없습니다: %1").arg(rightPath));
        leftFile.close();
        return;
    }
    ...
    if (!result.success) {
        ...
        if (fileType == "yaml") {
            ...
            QMessageBox::critical(
                    this, tr("Diff 오류"), QString::fromStdString(result.errorMessage));
        }
        return;
    }
                                     ...
                                     const QString &rightHeaderOverride)
{
    ...
    if (fileType == "yaml") {
        ...
        diffTable_->setHorizontalHeaderLabels(
                {"Line", "Path", "Left Value (Compare)", "Right Value (Base)", "State"});
        ...
        diffTable_->setColumnCount(4);
        diffTable_->setHorizontalHeaderLabels(
                {"Line", "Left Content (Compare)", "Right Content (Base)", "State"});
            ...
            if (result.isMember("changes") && result["changes"].isArray()) {
                ...
                for (Json::ArrayIndex i = 0; i < changes.size(); ++i) {
                    ...
                    if (change.isMember("oldValue") && !change["oldValue"].isNull()) {
                        ...
                        builder["indentation"] = "";
                        row.origin             = QString::fromStdString(
                                Json::writeString(builder, change["oldValue"]));
                        // JSON 따옴표 제거
                    ...
                    if (change.isMember("newValue") && !change["newValue"].isNull()) {
                        Json::StreamWriterBuilder builder;
                        builder["indentation"] = "";
                        row.target             = QString::fromStdString(
                                Json::writeString(builder, change["newValue"]));
                        // JSON 따옴표 제거
...
void ComparePage::performFolderDiff(const QString &leftPath, const QString &rightPath)
{
    ...
    services::ServiceResult result =
            services::DiffService::diffDirectories(finalRightPath.toStdString(),  // base (right)
                                                   finalLeftPath.toStdString()    // compare (left)
            );

    if (!result.success) {
        QMessageBox::critical(
                this, tr("폴더 비교 오류"), QString::fromStdString(result.errorMessage));
        return;
    }
