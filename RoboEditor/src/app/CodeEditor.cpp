...
void LineNumberArea::paintEvent(QPaintEvent* ev) { editor_->lineNumberAreaPaintEvent(ev); }
...
void CodeEditor::lineNumberAreaPaintEvent(QPaintEvent* ev) {
    ...
    while (blk.isValid() && top <= ev->rect().bottom()) {
        if (blk.isVisible() && bottom >= ev->rect().top()) {
            ...
            p.drawText(0, int(top), lineNumberArea_->width()-4,
                       fontMetrics().height(), Qt::AlignRight, QString::number(bn+1));
        }
        blk = blk.next(); top = bottom; bottom = top + blockBoundingRect(blk).height(); ++bn;
    }
}
...
void CodeEditor::highlightCurrentLine() {
    ...
    sel.cursor = textCursor(); sel.cursor.clearSelection();
    extra.append(sel);
