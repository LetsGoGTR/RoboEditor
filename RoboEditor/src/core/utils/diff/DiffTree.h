...
class DiffTree
{
  ...
  private:
    ...
    // 2) 폴더 간 파일(leaf) 비교
    static Result CompareDirs(const std::filesystem::path &left,
    ...
    struct SHA256
    {
        ...
        uint8_t                buf[64];
        size_t                 blen;
        static inline uint32_t R(uint32_t x, uint32_t n)
        {
            return (x >> n) | (x << (32U - n));
        }
        ...
        static inline uint32_t e0(uint32_t x)
        {
            return R(x, 2) ^ R(x, 13) ^ R(x, 22);
        }
        static inline uint32_t e1(uint32_t x)
        {
