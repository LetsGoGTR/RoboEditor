    ...
    struct TempDir
    {
        ...
        explicit TempDir(const char *tag)
        {
            ...
            for (int i = 0; i < 100; i++) {
                ...
                std::error_code ec;
                if (std::filesystem::create_directories(cand, ec)) {
                    p = cand;
                    return;
                }
            }
...
void DiffTree::SHA256::final(uint8_t out[32])
{
    ...
    while (blen < 56)
        buf[blen++] = 0;
    for (int i = 7; i >= 0; i--)
        buf[blen++] = static_cast<uint8_t>((bitlen >> (i * 8)) & 0xFFU);
    transform(buf);
    for (int i = 0; i < 8; i++) {
...
std::string DiffTree::Sha256OfFile(const std::filesystem::path &p)
{
    ...
    if (!in)
        throw std::runtime_error("open fail: " + p.string());
    SHA256 s;
    s.init();
    ...
    while (in) {
        ...
        if (n > 0)
            s.update(buf.data(), static_cast<size_t>(n));
    }
    uint8_t h[32];
    s.final(h);
    return ToHex(h, 32);
...
// 2) 폴더 간 파일(leaf) 비교
DiffTree::Result DiffTree::CompareDirs(const std::filesystem::path &left,
...
int DiffTree::RunCli(int argc, char **argv)
{
    ...
    try {
        ...
        if (!std::filesystem::exists(left) || !std::filesystem::is_directory(left)) {
            std::cerr << "Left not a directory: " << left << "\n";
            return 2;
        }
        if (!std::filesystem::exists(right) || !std::filesystem::is_directory(right)) {
            std::cerr << "Right not a directory: " << right << "\n";
            return 2;
        }
        ...
        if (changes.isArray() && changes.empty())
            return 0;
        return 3;
...
std::string DiffTree::HashSubtree(const std::filesystem::path &dir)
{
    ...
    for (; !ec && it != end; it.increment(ec)) {
        ...
        if (IsSymlinkEntry(de) || IsIgnoredName(name))
            continue;
...
bool DiffTree::IsArchivePath(const std::filesystem::path &p)
{
    ...
    std::transform(
            fn.begin(), fn.end(), fn.begin(), [](unsigned char c) { return std::tolower(c); });
...
void DiffTree::ExtractArchiveTo(const std::filesystem::path &src, const std::filesystem::path &out)
{
    ...
    while (archive_read_next_header(a, &entry) == ARCHIVE_OK) {
        ...
        if (type == AE_IFDIR) {
            ...
            char         buf[BS];
            la_ssize_t   r;
            while ((r = archive_read_data(a, buf, BS)) > 0)
                of.write(buf, r);
        } else {
