...
namespace diff_utils
{
  ...
  struct DiffEntry {
      ...
      // 매개변수 생성자
      DiffEntry(const std::string& t, const std::string& p, const Json::Value& old, 
                const Json::Value& newVal, int oldLine, int newLine, 
                int oldCount, int newCount)
          : type(t), path(p), oldValue(old), newValue(newVal), 
  ...
  void compareNodes(const YAML::Node& nodeA, const YAML::Node& nodeB, 
  ...
  // 추가된 차이점을 생성하는 독립 함수
  void addAddedDiff(const YAML::Node& node, const std::string& key, 
                  const std::string& path, std::vector<DiffEntry>& diffs);

  // 삭제된 차이점을 생성하는 독립 함수
  void addRemovedDiff(const YAML::Node& node, const std::string& key, 
                    const std::string& path, std::vector<DiffEntry>& diffs);

  // 수정된 차이점을 생성하는 독립 함수
  void addModifiedDiff(const YAML::Node& nodeA, const YAML::Node& nodeB, 
                      const std::string& path, std::vector<DiffEntry>& diffs);

  // 스칼라 값 비교 독립 함수
  void compareScalars(const YAML::Node& nodeA, const YAML::Node& nodeB, 
                    const std::string& path, std::vector<DiffEntry>& diffs);

  // Map 비교 독립 함수
  void compareMaps(const YAML::Node& nodeA, const YAML::Node& nodeB, 
                  const std::string& path, std::vector<DiffEntry>& diffs);

  // Sequence 비교 독립 함수
  void compareSequences(const YAML::Node& nodeA, const YAML::Node& nodeB, 
                      const std::string& path, std::vector<DiffEntry>& diffs);
  ...
  public:
    ...
    static std::vector<DiffEntry> compareFiles(const std::string& contentA, const std::string& contentB);
