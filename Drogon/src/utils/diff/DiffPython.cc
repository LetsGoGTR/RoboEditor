                                             ...
                                             /*out*/ std::vector<int>& origB)
{
    ...
    auto internSide = [&](const auto& src, std::vector<int>& I, std::vector<int>& orig){
        for (size_t i = 0; i < src.size(); ++i) {
            ...
            if (it == dict.end()) it = dict.emplace(sv, ++nextId).first;
                                  ...
                                  int x0,int x1,int y0,int y1, int vocabCap)
{
    ...
    for (const Cand& c : ca) buckA[Ai[c.pos]].push_back(c.pos);
    for (const Cand& c : cb) buckB[Bi[c.pos]].push_back(c.pos);
...
std::vector<std::pair<int, int>> DiffPython::extractAnchorsLIS(const std::vector<std::pair<int, int>>& pairs)
{
    if (pairs.empty()) return {};
    ...
    for (size_t idx = 0; idx < pairs.size(); ++idx) {
        ...
        if (k > 0) prevIdx[idx] = tailIdx[k - 1];
    }
