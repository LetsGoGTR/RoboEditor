                             ...
                             std::function<void(const drogon::HttpResponsePtr &)> &&callback)
{
    ...
    if (!json->isMember("serialNumber") || !json->isMember("name"))
        return sendError(callback, drogon::k400BadRequest, "Missing required fields: serialNumber, name");
    ...
    try {
        ...
        metadata.sftpPassword = json->isMember("sftpPassword") ? (*json)["sftpPassword"].asString() : "";
