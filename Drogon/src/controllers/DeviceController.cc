...
void api::v1::Device::update(
  ...
  try {
    ...
    auto existingResult =
        services::DeviceService::getDeviceInfo(baseDir, target);
    ...
    services::DeviceMetadata metadata;
    metadata.name = json->isMember("name")
                        ? (*json)["name"].asString()
  ...
  runningClient->sendRequest(
      ...
      [callbackPtr, ip, target, this](drogon::ReqResult result,
        ...
        if (result != drogon::ReqResult::Ok) {
          return sendError(*callbackPtr, drogon::k500InternalServerError,
                           "Failed to connect to robot", ip);
        }
        ...
        if (isRunning) {
          return sendError(*callbackPtr, drogon::k409Conflict,
                           "Robot is running",
        ...
        exportClient->sendRequest(
            exportReq, [callbackPtr, target, this](
              ...
              try {
                ...
                std::replace(timestampStr.begin(), timestampStr.end(), 'T',
                             '_');
                ...
                if (!importResult.success) {
                  return sendError(
                      *callbackPtr, drogon::k500InternalServerError,
                      "Failed to import workspace", importResult.errorMessage);
                }
                ...
                sendError(*callbackPtr, drogon::k500InternalServerError,
                          "Internal server error", e.what());
              }
            });
      });
