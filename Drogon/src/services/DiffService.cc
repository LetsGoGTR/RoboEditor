...
const std::vector<std::string> services::DiffService::supportedFormats_ = {
    ".yaml", ".yml",  ".srl", ".py", ".txt", ".log",
    ".cfg",  ".conf", ".ini", ".md", ".json"};
...
services::DiffResult services::DiffService::diffPython(
...
services::DiffResult services::DiffService::diffText(
    const std::string& contentA, const std::string& contentB,
    const std::string& nameA, const std::string& nameB) {
  services::DiffResult result;
  try {
...
services::DiffResult services::DiffService::diff(const std::string& contentA,
  ...
  if (extA != extB) {
    result.errorMessage =
        "File extensions do not match: " + extA + " vs " + extB;
