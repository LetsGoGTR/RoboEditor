...
const std::string services::WorkspaceService::metadataFilename_ =
    ".workspace.json";

// Helper functions
services::WorkspaceOperationResult services::WorkspaceService::createError(
...
services::WorkspaceMetadata services::WorkspaceMetadata::fromJson(
    const Json::Value& json) {
  WorkspaceMetadata metadata;

  if (json.isMember("id")) metadata.id = json["id"].asString();
  if (json.isMember("target")) metadata.target = json["target"].asString();
...
bool services::WorkspaceService::saveMetadata(
    const std::string& workspacePath, const WorkspaceMetadata& metadata) {
  try {
...
services::WorkspaceMetadata services::WorkspaceService::loadMetadata(
...
services::WorkspaceOperationResult services::WorkspaceService::importWorkspace(
  ...
  try {
    ...
    while (archive_read_next_header(a, &entry) == ARCHIVE_OK) {
      std::string fullPath =
          workspacePath + "/" + archive_entry_pathname(entry);
      ...
      if (archive_write_header(ext, entry) == ARCHIVE_OK &&
        ...
        size_t size;
        int64_t offset;
        while (archive_read_data_block(a, &buff, &size, &offset) ==
               ARCHIVE_OK) {
    ...
    LOG_INFO << "Imported workspace: " << metadata.name
             << " (ID: " << metadata.id << ")";
    return result;

  } catch (const std::exception& e) {
    if (fs::exists(workspacePath)) fs::remove_all(workspacePath);
...
services::WorkspaceOperationResult services::WorkspaceService::exportWorkspace(
  ...
  try {
    ...
    for (const auto& dirEntry :
         fs::recursive_directory_iterator(workspacePath)) {
      if (!dirEntry.is_regular_file()) continue;

      std::string relativePath =
          fs::relative(dirEntry.path(), workspacePath).string();
      if (relativePath == metadataFilename_) continue;  // Skip metadata
    ...
    LOG_INFO << "Exported workspace: " << metadata.name
             << " (ID: " << workspaceId << ")";
    return result;
...
services::WorkspaceOperationResult services::WorkspaceService::listWorkspaces(
    const std::string& baseDir) {
  if (!fs::exists(baseDir)) {
    try {
      fs::create_directories(baseDir);
    } catch (const std::exception& e) {
      return createError("Failed to create base directory: " +
                         std::string(e.what()));
    }
  }
  ...
  try {
    ...
    for (const auto& entry : fs::directory_iterator(baseDir)) {
      ...
      if (metadata.id.empty()) continue;  // Skip invalid
