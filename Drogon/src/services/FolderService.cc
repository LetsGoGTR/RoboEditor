...
bool services::FolderService::validateFolderPath(
...
Json::Value services::FolderService::getFolderInfo(
    const std::string& folderPath) {
  Json::Value info;
...
services::FolderOperationResult services::FolderService::createFolder(
    const std::string& folderPath) {
...
services::FolderOperationResult services::FolderService::updateFolder(
  ...
  try {
    ...
    LOG_INFO << "Successfully renamed folder from " << oldPath << " to "
             << newPath;
...
services::FolderOperationResult services::FolderService::deleteFolder(
    const std::string& folderPath) {
  services::FolderOperationResult result;
  result.success = false;
...
services::FolderOperationResult services::FolderService::readFolder(
    const std::string& folderPath) {
  services::FolderOperationResult result;
  result.success = false;
