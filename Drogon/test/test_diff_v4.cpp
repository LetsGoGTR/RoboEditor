...
static std::vector<Diff> computeDiffs(const std::vector<NormalizedLine> &aNorm,
...
TEST(NormalizeTest, TC004_LongLineMixedWhitespace_ToSame)
{
    ...
    auto diffs = computeDiffs(
            DiffPython::normalizeAll(Aline), DiffPython::normalizeAll(Bline), Aline, Bline);
...
TEST(DiffEngineTest, TC101_AllLinesIdentical_AllSAME)
{
    ...
    auto diffs = computeDiffs(DiffPython::normalizeAll(contentA),
...
TEST(DiffEngineTest, TC109_AddDelSymmetry_Holds)
{
    ...
    auto ab = computeDiffs(DiffPython::normalizeAll(contentA),
                           DiffPython::normalizeAll(contentB),
                           contentA,
                           contentB);
    auto ba = computeDiffs(DiffPython::normalizeAll(contentB),
...
TEST(JsonBuildTest, TC201_JSONStructure_ExactMatch)
{
    ...
    // 완전 구조 비교
    EXPECT_EQ(actualChanges, expected) << "Actual JSON:\n"
...
teeq '''
    ...
    auto assertModJson = [](const Json::Value &it,
                            int                expBaseLine1,
                            const std::string &expBaseValue,
                            int                expCompLine1,
                            const std::string &expCompValue) {
        EXPECT_EQ(it["type"].asString(), "modified");
