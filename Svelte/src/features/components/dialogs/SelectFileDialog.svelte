<script lang="ts">
  ...
  let {
    ...
    onConfirm: (payload: { name: string; file: FileNode }) => void;
    onCancel: () => void;
  }>();
  ...
  function handleSelect(node: TreeNode) {
    if (node.type === "file") selectedFile = node;
  }

  function confirm() {
    const name = filename.trim();
    if (!name || !selectedFile) return;
    onConfirm({ name, file: selectedFile });
    close();
  }
  ...
  <dialog open class="dialog-box">
    ...
    <div class="form-row">
      ...
      <input
        ...
        class="path-input"
        readonly
        value={selectedFile ? selectedFile.path : ""}
      />
    </div>
