<script lang="ts">
	...
	function applyDiff() {
		...
		const original = getOrCreateModel(toMonacoUri(diff.leftFilePath), diff.leftContent);
		const modified = getOrCreateModel(toMonacoUri(diff.rightFilePath), diff.rightContent);
